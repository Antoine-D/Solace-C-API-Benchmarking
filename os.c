...
_getDateTime ( char *buf_p, int bufSize )
{
    ...
    mseconds = (int) ( ( tv.tv_usec + 500 ) / 1000 );
    snprintf ( buf_p, bufSize, "%s.%03d %d", buffer, mseconds, 1900 + timeStruct.tm_year );
#endif
...
waitUntil ( UINT64 nexttimeInUs )
{
    ...
    struct timeval  tv;
    tv.tv_sec = 0;
    tv.tv_usec = (suseconds_t) waitTime;
    select ( 0, 0, 0, 0, &tv );
#endif
    return waitTime;
}
         ...
         )
{
    ...
    if ( timeoutSec < 0 ) {
        ...
        gettimeofday (&absTimeval, NULL);
        absTimeout.tv_sec = absTimeval.tv_sec + timeoutSec;
        absTimeout.tv_nsec = absTimeval.tv_usec * 1000;
